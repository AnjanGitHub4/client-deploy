...
export default function Marquee({styles}) {
  ...
  return (
    <section className="py-6 lg:py-12 relative overflow-hidden bg-gradient-to-r from-blue-600 to-purple-600 z-0">
      <h3 className={`w-full text-center absolute top-0 left-[50%] translate-x-[-50%] py-2 font-bold text-3xl lg:text-7xl text-green-900 ${styles} z-0`}>
        MOSTLY VISIT
      </h3>
      <Container width={90}>
        <div className="relative flex items-center justify-center py-6 overflow-hidden z-10">
          <div className="absolute top-0 flex animate-marquee whitespace-nowrap overflow-hidden">
          ...
          <div className="absolute top-0 flex animate-marquee-reverse whitespace-nowrap overflow-hidden">
            {destinations.map((item, i) => (
              <div
                key={`item-${i}`}
